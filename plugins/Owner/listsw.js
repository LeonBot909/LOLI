const handler = async (m, { conn }) => {
  ...
  if (list.length === 0) {
    return await conn.reply(
      m.chat,
      `Tidak ada cerita yang tersedia saat ini. Silakan tambahkan cerita dengan mengirim gambar, video, atau pesan suara.`,
      m,
      { mentions: [m.sender] }
    );
  }
  ...
  const formatMessage = (obj, index) => {
    ...
    let text = `*${
      index + 1
    }.* ${messageType.toUpperCase()} - @${senderUsername}`;
    if (caption) text += `\n${caption}`;
    return text + `\n`;
  };
...
handler.before = async (m, { conn }) => {
  ...
  if (isNaN(index) || index < 1 || index > list.length) {
    ...
    if (
      selectedObj.type === "imageMessage" ||
      selectedObj.type === "videoMessage"
    ) {
