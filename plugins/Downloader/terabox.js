...
const handler = async (m, { conn, args }) => {
  ...
  try {
    ...
    if (!args[1] || selectedFileIndex < 0 || selectedFileIndex >= res.length) {
      return conn.reply(
        m.chat,
        "⚠️ Masukkan nomor yang valid. Contoh: .terabox url 1 (1 sampai " +
          res.length +
          ")",
        m
      );
    ...
    if (fileInfo.size > sizeLimit) {
      return conn.reply(
        m.chat,
        "⚠️ Ukuran file melebihi batas 100MB. Ukuran file: " + formattedSize,
        m
      );
    }
    if (fileInfo.ok) {
...
async function fetchInfo(shortUrl, pwd = "") {
  try {
    const response = await got(
      `https://terabox-dl.qtcloud.workers.dev/api/get-info?shorturl=${shortUrl}&pwd=${pwd}`,
      {
        responseType: "json",
      }
    );
