...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if (
    Object.values(conn.judipvp).find(
      (room) =>
        room.id.startsWith("judipvp") && [room.p, room.p2].includes(m.sender)
    )
  )
    throw "Selesaikan judi mu yang sebelumnya";
  if (
    Object.values(conn.judipvp).find(
      (room) =>
    ...
    throw `Orang yang kamu tantang sedang bermain judipvp bersama orang lain :(`;
  ...
  let item = items.filter(
    (v) => v in user[m.sender] && typeof user[m.sender][v] == "number"
  );
  ...
  if (!item.includes(type))
    return m.reply("Item yang tersedia\n• Money\n• Chip");
  if (!count || !musuh)
    return m.reply(
      `Masukan format dengan benar\n\nContoh :\n${
        usedPrefix + command
      } money 10000 ${m.sender.split("@")[0]}`
    );
  conn.judipvp[id] = {
    chat: await conn.reply(
      m.chat,
      `@${m.sender.split("@")[0]} Mengajak @${
        musuh.split("@")[0]
      } Berjudi Apakah Kamu Mau Menerimanya? (Y/N)`,
      m,
      {
