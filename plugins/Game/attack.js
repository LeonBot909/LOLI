let handler = async (m, { conn, usedPrefix, args, command }) => {
  ...
  async function cekAFK(x) {
    ...
    await sleep(90000);
    let turnNow = conn.war2[m.chat] ? conn.war2[m.chat].turn : "";
    let timeNow = conn.war2[m.chat] ? conn.war2[m.chat].time : "tidak ada";
    ...
    if (turn == turnNow && time == timeNow) {
      ...
      conn.reply(
        m.chat,
        `*@${
          conn.war[m.chat][turn].user.split("@")[0]
        } sedang AFK (Denda -2500 HP)*\n\n.war player = statistik pemain\n.attack @tag = serang lawan`,
        null,
        { contextInfo: { mentionedJid: [conn.war[m.chat][turn].user] } }
      );
      await sleep(3000);
      // cek kalau mati
          ...
          if (playerTotal > 0 && playerTotal == playerKalah) {
            ...
            for (let j = 0; j < 5; j++) {
              if (conn.war[m.chat][j].user != "") {
                global.db.data.users[conn.war[m.chat][j].user].money -= Number(
                  conn.war2[m.chat].money
                );
      ...
      if (turn < 5) {
        for (let i = 5; i < 10; i++) {
          if (
            conn.war[m.chat][i].hp > 0 &&
            conn.war[m.chat][i].user != "" &&
      ...
      conn.reply(
        m.chat,
        `*Giliran @${
          conn.war[m.chat][conn.war2[m.chat].turn].user.split("@")[0]
      ...
      if (conn.war[m.chat][i].user == args[0] && conn.war[m.chat][i].hp > 0) {
        ...
        for (let i = 0; i < 10; i++) {
          if (conn.war[m.chat][i].user == target) {
            ...
            conn.reply(
              m.chat,
              conn.war[m.chat][conn.war2[m.chat].turn].turn,
              m
            );
            conn.war[m.chat][conn.war2[m.chat].turn].turn = true;
            conn.reply(
  ...
  if (conn.war2[m.chat].turn < 5) {
        ...
        `*TEAM A MENANG KARENA TEAM B GOBLOK SEMUA*\n\n*TEAM A :*\n` +
          teamA.map(
            (v, i) =>
              `${conn.war[m.chat][i].hp > 0 ? "❤️ " : "☠️ "}@${
                v.split("@")[0]
              } (+ Rp. ${Number(conn.war2[m.chat].money).toLocaleString()})`
          ).join`\n` +
          "\n\n*TEAM B :*\n" +
          teamB.map(
            (v, i) =>
    ...
    for (let k = 5; k < 10; k++) {
      if (
        conn.war[m.chat][k].hp > 0 &&
        conn.war[m.chat][k].user != "" &&
        conn.war[m.chat][k].turn == false
      ) {
    ...
    let turn1 = conn.war2[m.chat] ? conn.war2[m.chat].turn : "";
    let turn2 = conn.war2[m.chat] ? conn.war2[m.chat].turn : "";
    for (let k = 0; k < 5; k++) {
      if (
        conn.war[m.chat][k].hp > 0 &&
        conn.war[m.chat][k].user != "" &&
