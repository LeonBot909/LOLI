...
*${usedPrefix + command} start* = start game`);
  ...
  if (args[0] == "money") {
    ...
    if (m.sender == conn.war[m.chat][0].user) {
      ...
      return conn.reply(
        m.chat,
        `*Hanya @${
          conn.war[m.chat][0].user.split("@")[0]
    ...
    if (!(m.chat in conn.war)) {
      ...
      if (args[1]) {
        if (args[1].toLowerCase() == "a") {
          ...
          return m.reply("a");
          if (global.db.data.users[m.sender].money < conn.war2[m.chat].money)
            return m.reply(
              `*Uang kamu minimal Rp. ${conn.war2[
                m.chat
  ...
  if (args[0] == "player") {
    ...
    conn.reply(
      m.chat,
      `${
        conn.war2[m.chat].war
          ? "*Giliran : " +
            "@" +
            conn.war[m.chat][conn.war2[m.chat].turn].user.split("@")[0] +
            "*\n*Taruhan : Rp. " +
            Number(conn.war2[m.chat].money).toLocaleString() +
            "*\n\n"
          : "*Taruhan : Rp. " +
            Number(conn.war2[m.chat].money).toLocaleString() +
            "*\n\n"
        ...
        teamA.map(
          (v, i) =>
            `${conn.war[m.chat][i].hp > 0 ? "❤️ " : "☠️ "}@${
              v.split("@")[0]
            } (Lv.${conn.war[m.chat][i].lvl} HP: ${conn.war[m.chat][i].hp})`
        ).join`\n` +
        "\n\n*TEAM B :*\n" +
        teamB.map(
          (v, i) =>
            `${conn.war[m.chat][i + 5].hp > 0 ? "❤️ " : "☠️ "}@${
              v.split("@")[0]
            } (Lv.${conn.war[m.chat][i + 5].lvl} HP: ${
              conn.war[m.chat][i + 5].hp
            })`
        ).join`\n`,
      m,
      {
  ...
  // START GAME
  if (args[0] == "start") {
    if (conn.war2[m.chat] && conn.war2[m.chat].war)
      return m.reply(`*Perang sudah dimulai, tidak bisa join.*`);
    teamA = 0;
    teamB = 0;
    for (let i = 0; i < 10; i++) {
      if (i < 5) {
        if (conn.war[m.chat] && conn.war[m.chat][i].user != "") teamA += 1;
      } else {
        if (conn.war[m.chat] && conn.war[m.chat][i].user != "") teamB += 1;
      }
    }
